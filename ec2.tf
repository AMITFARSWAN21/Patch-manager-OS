...
resource "aws_instance" "ubuntu22_ssm_test" {
  ami                    = "ami-0f457776b2f2411c1"   # ubuntu-jammy-22.04 - 2025-11-11 (oldest)
  instance_type          = "t3.medium"
  subnet_id              = aws_subnet.private.id
  vpc_security_group_ids = [
  ...
  user_data = <<-EOF
    ...
    if [ "$READY" = "false" ]; then
      echo "WARNING: apt-cache server not reachable after 30 attempts" >> /var/log/ssm-init.log
    fi

    # Configure apt to use apt-cache server
...
resource "aws_instance" "ubuntu24_ssm_test" {
  ami                    = "ami-036d2bb3f14d36e07"   # ubuntu-noble-24.04 - 2025-11-26 (oldest)
  instance_type          = "t3.medium"
  subnet_id              = aws_subnet.private.id
  vpc_security_group_ids = [
    aws_security_group.ssm_ubuntu_sg.id,
...
output "ubuntu22_instance_id" {
  description = "Instance ID of the Ubuntu 22.04 SSM test instance"
  value       = aws_instance.ubuntu22_ssm_test.id
}

output "ubuntu22_instance_private_ip" {
  description = "Private IP of the Ubuntu 22.04 SSM test instance"
...
output "ubuntu24_instance_id" {
  description = "Instance ID of the Ubuntu 24.04 SSM test instance"
  value       = aws_instance.ubuntu24_ssm_test.id
}

output "ubuntu24_instance_private_ip" {
  description = "Private IP of the Ubuntu 24.04 SSM test instance"
